#!/usr/bin/env python3
"""
ZeroClaw Agent - Ultra-minimal Stateless Explorer (Abel's architecture)
Version 2.0 - Production implementation

Features:
- Ultra-minimal stateless agent
- 60s timeouts, 100-token responses
- No persistent memory (fresh each session)
- Brief, terse observations
- Config-driven
...
def fetch(url):
    try:
        r = urllib.request.urlopen(urllib.request.Request(url, headers={'User-Agent': 'ZeroClaw/2'}), timeout=TIMEOUT)
